...
class DiscoverPageState extends State<DiscoverPage> {
  ...
  // --- Fonksiyonel Kod Bloğu (Değişiklik Yok) ---
  @override
  ...
  Future<void> _showRandomInsiderTip(City city) async { final foodTips = await DatabaseHelper.instance.getFoodTipsForCity(city.id); if (foodTips.isNotEmpty) { final randomFoodTip = foodTips[Random().nextInt(foodTips.length)]; await Future.delayed(const Duration(milliseconds: 1000)); if (!mounted) return; showDialog( context: context, barrierDismissible: false, builder: (BuildContext context) { return BackdropFilter( filter: ImageFilter.blur(sigmaX: 3, sigmaY: 3), child: AlertDialog( shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15)), title: const Text("Insider Tip!", textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 22)), content: Column( mainAxisSize: MainAxisSize.min, children: [ Text( randomFoodTip.foodDisplayName, textAlign: TextAlign.center, style: TextStyle( fontSize: 18, fontWeight: FontWeight.w600, color: Theme.of(context).colorScheme.primary, ), ), const SizedBox(height: 16), Text( randomFoodTip.tip, textAlign: TextAlign.center, style: const TextStyle(fontSize: 16, height: 1.4), ), ], ), actions: <Widget>[ TextButton( child: const Text("Got it!", style: TextStyle(fontWeight: FontWeight.bold)), onPressed: () { Navigator.of(context).pop(); }, ), ], ), ); }, ); } }
  Future<void> startChatFlowForCity(City city) async { _showRandomInsiderTip(city); setState(() { _isTransitioning = false; _chatMessages.clear(); _chatMessages.add(TypingIndicatorMessage()); _selectedCity = city; }); _scrollToBottom(); await Future.delayed(const Duration(milliseconds: 1200)); final greeting = city.greetingsEn ?? "Welcome to ${city.cityName}!"; _addBotMessage(greeting, onFinished: () async { final localFoods = await DatabaseHelper.instance.getFoodsForCity(city.id); if (localFoods.isNotEmpty) { _addBotMessage("When in ${city.cityName}, you absolutely must try these local specialties:", onFinished: () { setState(() { _chatMessages.add(FoodSuggestionMessage(localFoods, _onFoodTapped)); }); _scrollToBottom(); _offerNextSteps(city); }); } else { _addBotMessage("I'm still learning about the local specialties of ${city.cityName}. But, we can explore the entire Turkish cuisine together!", onFinished: () { _showTurkishCuisineCategories(); }); } }); }
  void _offerNextSteps(City city) { Future.delayed(const Duration(milliseconds: 1500), () { if (!mounted) return; setState(() { _chatMessages.removeWhere((msg) => msg is ButtonOptionsMessage); _chatMessages.add(ButtonOptionsMessage( title: "What would you like to do now?", options: [ ChatButtonOption( text: "📍 Find places for local food", icon: Icons.restaurant_menu, onPressed: () => _handleNavigateToVenueExplorer(city) ), ChatButtonOption( text: "🇹🇷 Explore all Turkish Cuisine", icon: Icons.travel_explore, onPressed: _handleExploreTurkishCuisine ), ChatButtonOption( text: "ℹ️ More about ${city.cityName}", icon: Icons.info_outline, onPressed: _showCityContextOptions ), ] )); }); _scrollToBottom(); }); }
  void _handleNavigateToVenueExplorer(City city) { Navigator.push( context, MaterialPageRoute( builder: (context) => VenueExplorerPage( city: city, showAllTurkishFoods: false, ), ), ); }
  void _handleExploreTurkishCuisine() { setState(() { _chatMessages.removeWhere((msg) => msg is ButtonOptionsMessage); _chatMessages.add(UserTextMessage("Explore Turkish Cuisine")); }); _scrollToBottom(); _addBotMessage("Great choice! The culinary map of Turkey is vast and delicious. Which category interests you the most?", onFinished: () { _showTurkishCuisineCategories(); }); }
  void _showTurkishCuisineCategories() { setState(() { final options = _turkishCuisineCategories.entries.map((entry) { return ChatButtonOption( text: entry.value, icon: null, onPressed: () => _handleTurkishCategorySelection(entry.key, entry.value), ); }).toList(); _chatMessages.add(ButtonOptionsMessage(options: options)); }); _scrollToBottom(); }
  Future<void> _handleTurkishCategorySelection(String categoryKey, String categoryName) async { setState(() { _chatMessages.removeWhere((msg) => msg is ButtonOptionsMessage); _chatMessages.add(UserTextMessage(categoryName)); _chatMessages.add(TypingIndicatorMessage()); }); _scrollToBottom(); final foodsInCategory = await DatabaseHelper.instance.getFoodsByCategory(categoryKey); await Future.delayed(const Duration(milliseconds: 1200)); if (foodsInCategory.isNotEmpty) { _addBotMessage("Here are some of the most beloved dishes from the '$categoryName' category across Turkey:", onFinished: () { setState(() { _chatMessages.add(FoodSuggestionMessage(foodsInCategory, _onFoodTapped)); }); _scrollToBottom(); _offerVenueExplorerForTurkishCuisine(categoryKey); }); } else { _addBotMessage("I couldn't find any dishes for the '$categoryName' category at the moment. Please try another one!", onFinished: (){ _showTurkishCuisineCategories(); }); } }
  void _offerVenueExplorerForTurkishCuisine(String categoryKey) { Future.delayed(const Duration(milliseconds: 1500), () { if (!mounted) return; setState(() { final city = _selectedCity!; _chatMessages.add(ButtonOptionsMessage( title: "Found something you like? Let's find a place!", options: [ ChatButtonOption( text: "📍 Find places for '${_turkishCuisineCategories[categoryKey]}'", icon: Icons.search, onPressed: () { Navigator.push( context, MaterialPageRoute( builder: (context) => VenueExplorerPage( city: city, selectedCategory: categoryKey, ), ), ); } ), ChatButtonOption( text: "⬅️ Back to Categories", icon: Icons.category_outlined, onPressed: () { setState(() { _chatMessages.removeWhere((msg) => msg is ButtonOptionsMessage); }); _addBotMessage("Sure, which other category would you like to see?", onFinished: () { _showTurkishCuisineCategories(); }); } ), ChatButtonOption( text: "ℹ️ More about ${city.cityName}", icon: Icons.info_outline, onPressed: _showCityContextOptions ), ] )); }); _scrollToBottom(); }); }
  void _showCityContextOptions() { Future.delayed(const Duration(milliseconds: 500), () { if (!mounted) return; setState(() { _chatMessages.add(ButtonOptionsMessage( title: "Want to know more about ${ _selectedCity!.cityName }'s culinary secrets?", options: [ ChatButtonOption(text: "Food Culture", icon: Icons.auto_stories_outlined, onPressed: () => _handleContextQuery("culture", "Food Culture")), ChatButtonOption(text: "Local Drinks", icon: Icons.local_cafe_outlined, onPressed: () => _handleContextQuery("drinks", "Local Drinks")), ChatButtonOption(text: "Iconic Dish", icon: Icons.star_border_outlined, onPressed: () => _handleContextQuery("iconic_dish", "Iconic Dish")), ChatButtonOption(text: "After the Meal", icon: Icons.directions_walk_outlined, onPressed: () => _handleContextQuery("after_meal", "After the Meal")), ], )); }); _scrollToBottom(); }); }
  void _handleContextQuery(String queryType, String userText) { setState(() { _chatMessages.removeWhere((msg) => msg is ButtonOptionsMessage); _chatMessages.add(UserTextMessage(userText)); _chatMessages.add(TypingIndicatorMessage()); }); _scrollToBottom(); Future.delayed(const Duration(milliseconds: 1200), () { if(!mounted) return; final city = _selectedCity!; String? responseText; switch (queryType) { case "culture": responseText = city.cultureSummaryEn; _addBotMessage(responseText ?? "I'm still learning about the unique food culture of ${city.cityName}.", onFinished: () => _offerNextSteps(city)); break; case "drinks": responseText = city.localDrinksEn; _addBotMessage(responseText ?? "While I don't have specific drink recommendations, Ayran is a popular choice all over Turkey!", onFinished: () => _offerNextSteps(city)); break; case "after_meal": responseText = city.postMealSuggestionsEn; _addBotMessage(responseText ?? "A short walk and a Turkish coffee is always a great idea after a good meal!", onFinished: () => _offerNextSteps(city)); break; case "iconic_dish": _showIconicDish(city); break; } }); }
  Future<void> _showIconicDish(City city) async { if (city.iconicDishName == null || city.iconicDishName!.isEmpty) { _addBotMessage("It's hard to pick just one! Every dish in ${city.cityName} tells a part of its story.", onFinished: () => _offerNextSteps(city)); return; } final iconicDishDetails = await DatabaseHelper.instance.getFoodByName(city.iconicDishName!); if (iconicDishDetails != null) { _addBotMessage("When in ${city.cityName}, you absolutely must try its most iconic dish: ${iconicDishDetails.turkishName}!", onFinished: () { setState(() { _chatMessages.add(FoodSuggestionMessage([iconicDishDetails], _onFoodTapped)); }); _scrollToBottom(); Future.delayed(const Duration(milliseconds: 500), () => _offerNextSteps(city)); } ); } else { _addBotMessage("I know the most iconic dish is '${city.iconicDishName}', but I couldn't find its details right now.", onFinished: () => _offerNextSteps(city)); } }
  // --- Bitiş: Fonksiyonel Kod Bloğu ---
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        // TEMA DEĞİŞİKLİĞİ: backgroundColor kaldırıldı, artık main.dart'tan geliyor.
        leading: 
          _selectedCity != null 
            ? IconButton(
  ...
  Widget _buildJourneyStartMessage(City city) {
    return Center(
      child: Container(
        ...
        // TEMA DEĞİŞİKLİĞİ: Mavi tonları yerine turuncu tonları kullanıldı.
        decoration: BoxDecoration(gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [ Colors.deepOrange.shade50, Colors.deepOrange.shade100.withOpacity(0.8) ]), borderRadius: BorderRadius.circular(20), boxShadow: [ BoxShadow(color: Theme.of(context).colorScheme.primary.withOpacity(0.1), blurRadius: 20, spreadRadius: 5) ]), 
        child: Column(mainAxisSize: MainAxisSize.min, children: [ 
          TweenAnimationBuilder(duration: const Duration(milliseconds: 1200), tween: Tween<double>(begin: 0, end: 1), builder: (context, double value, child) { 
            return Transform.scale(scale: 0.8 + (value * 0.2), child: Transform.rotate(angle: value * 0.1, child: Hero(tag: 'city-pin-${city.id}', child: Material(type: MaterialType.transparency, child: Container(
              padding: const EdgeInsets.all(16), 
              // TEMA DEĞİŞİKLİĞİ: Mavi tonları yerine turuncu tonları kullanıldı.
              decoration: BoxDecoration(shape: BoxShape.circle, gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [ Theme.of(context).colorScheme.secondary, Theme.of(context).colorScheme.primary ]), boxShadow: [ BoxShadow(color: Theme.of(context).colorScheme.primary.withOpacity(0.3), blurRadius: 15, spreadRadius: 2) ]), 
              child: const Icon(Icons.explore, size: 40, color: Colors.white)
            ))))); 
          }), 
          const SizedBox(height: 20), 
          TweenAnimationBuilder(duration: const Duration(milliseconds: 800), tween: Tween<double>(begin: 0, end: 1), builder: (context, double value, child) { 
            // TEMA DEĞİŞİKLİĞİ: Metin rengi temadan alındı.
            return Opacity(opacity: value, child: Transform.translate(offset: Offset(0, 20 * (1 - value)), child: Text("Exploring ${city.cityName}", style: TextStyle(fontSize: 24, color: Theme.of(context).colorScheme.primary, fontWeight: FontWeight.bold, letterSpacing: 1.2), textAlign: TextAlign.center))); 
          }), 
          const SizedBox(height: 12), 
          TweenAnimationBuilder(duration: const Duration(milliseconds: 1000), tween: Tween<double>(begin: 0, end: 1), builder: (context, double value, child) { 
         ...
         const LoadingAnimation(size: 80),
          ...
          TweenAnimationBuilder(duration: const Duration(milliseconds: 1500), tween: Tween<double>(begin: 0, end: 1), builder: (context, double value, child) { 
            return Row(mainAxisAlignment: MainAxisAlignment.center, children: List.generate(3, (index) { 
              ...
              // TEMA DEĞİŞİKLİĞİ: Noktaların rengi temadan alındı.
              return Container(margin: const EdgeInsets.symmetric(horizontal: 4), child: AnimatedContainer(duration: Duration(milliseconds: 300 + (index * 100)), width: 8, height: 8, decoration: BoxDecoration(shape: BoxShape.circle, color: Theme.of(context).colorScheme.primary.withOpacity(animValue)))); 
            })); 
          })
        ])
      )
...
class CityPin extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // TEMA DEĞİŞİKLİĞİ: Pin rengini kırmızı yerine temanın ana rengi yapıyoruz.
    return Hero(tag: 'city-pin-${city.id}', child: Material(type: MaterialType.transparency, child: Column(mainAxisSize: MainAxisSize.min, children: [ Container(padding: EdgeInsets.symmetric(horizontal: 6 * scaleFactor, vertical: 2 * scaleFactor), decoration: BoxDecoration(color: Colors.black.withOpacity(0.5), borderRadius: BorderRadius.circular(8 * scaleFactor)), child: Text(city.cityName, style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: baseFontSize * scaleFactor))), Icon(Icons.location_on, color: Theme.of(context).colorScheme.primary, size: baseIconSize * scaleFactor, shadows: [Shadow(color: Colors.black54, blurRadius: 4.0 * scaleFactor)])])));
  }
}