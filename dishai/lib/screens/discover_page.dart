...
class DiscoverPageState extends State<DiscoverPage> with TickerProviderStateMixin {
  ...
  ScrollController? _scrollController; // <-- DÜZELTME 1: Nullable yapıldı
  City? _selectedCity;
  ...
  @override
  void dispose() {
    _scrollController?.dispose(); // <-- DÜZELTME 4: Ana dispose'da da kontrol et
  ...
  void _resetToMapView() {
    _scrollController?.dispose(); // <-- DÜZELTME 3: Controller'ı yok et
  ...
  void _scrollToBottom() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (_scrollController?.hasClients ?? false) { // <-- DÜZELTME 5: Null kontrolü
  ...
  Future<void> _loadCitiesForMap() async { 
      ...
      while (cities.isEmpty && retries > 0 && mounted) { 
          ...
          cities = await DatabaseHelper.instance.getAllCities(); retries--; 
      } 
      if (!mounted) return; 
      if (cities.isEmpty) { 
          setState(() { 
  ...
  Future<void> startChatFlowForCity(City city) async {
    ...
    _scrollController?.dispose(); // <-- DÜZELTME 2: Controller'ı oluştur
  ...
  // ... (Diğer tüm metotlar aynı kalacak: _offerNextSteps, _handle..., _show...)
  
  // ... (_offerNextSteps ve diğer tüm chat mantığı fonksiyonları burada, onlarda değişiklik yok)
  
  // Sadece ilgili fonksiyonları buraya kopyalıyorum
  ...
  Future.delayed(const Duration(milliseconds: 1500), () {
    ...
    setState(() {
      ...
      _chatMessages.add(ButtonOptionsMessage(
        ...
        options: [
          ChatButtonOption(
            text: "📍 Find places for local food",
...
Future<void> _showIconicDish(City city) async {
  ...
  // ... (YENİ _buildMapView METODU BURADA, IgnorePointer düzeltmesiyle birlikte)
      ...
      children: [
              ...
              child: Container(
                decoration: BoxDecoration(
                  image: DecorationImage(
                    image: AssetImage(_sceneImages[_currentSceneIndex]),
                    fit: BoxFit.cover,
              ...
              children: [
                Text(
                  ...
                  style: GoogleFonts.playfairDisplay(
                    ...
                    letterSpacing: 1.2,
                    shadows: [const Shadow(blurRadius: 12.0, color: Colors.black54)],
                  ),
                ),
                const SizedBox(height: 12),
                Text(
                  "Every city tells a story, every dish holds a memory.\nSelect a city to begin your journey.",
                  textAlign: TextAlign.center,
        ...
        if (_isFineTuningMode) 
          Positioned(
            ...
            left: 0,
            right: 0,
            child: Container(padding: const EdgeInsets.all(12), color: Colors.grey.shade200.withOpacity(0.9), child: Column(children: [ 
              Text('İnce Ayar Modu: ${_selectedCityForTuning ?? "Konumu ayarlamak için bir şehir seçin."}', style: const TextStyle(fontWeight: FontWeight.bold)), 
  ...
  Widget _buildTransitionOverlay() { 
    // ... Bu metodun içeriği aynı, değişiklik yok
  ...
  Widget _buildJourneyStartMessage(City city) { 
    // ... Bu metodun içeriği aynı, Lottie animasyonu ile birlikte
  ...
  Widget _buildChatView() {
    if (_scrollController == null) return const Column(); // <-- DÜZELTME 5: Güvenlik kontrolü

    return Column(children: [
      Expanded(
        child: ListView.builder(
          controller: _scrollController!, // <-- '!' artık güvenli
  ...
  // ... (Diğer tüm _build... metotları ve CityPin widget'ı aynı kalacak)
