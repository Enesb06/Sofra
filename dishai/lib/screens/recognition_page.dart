...
class StreamingTextMessage extends ChatMessage { final String text; final VoidCallback onFinished; StreamingTextMessage(this.text, {required this.onFinished}) : super(false); }
class ButtonOptionsMessage extends ChatMessage { final List<ChatButtonOption> options; ButtonOptionsMessage(this.options) : super(false); }
class ChatButtonOption { final String text; final VoidCallback onPressed; ChatButtonOption({required this.text, required this.onPressed}); }
class TypingIndicatorMessage extends ChatMessage { TypingIndicatorMessage() : super(false); }
class CustomWidgetMessage extends ChatMessage { final Widget child; CustomWidgetMessage(this.child) : super(false); }
...
class _RecognitionPageState extends State<RecognitionPage> {
  ...
  void _showPronunciation() {
    ...
    setState(() { _isBotTyping = false; _chatMessages.add(CustomWidgetMessage(pronunciationMessage)); });
    _scrollToBottom();
    Future.delayed(const Duration(milliseconds: 500), _askForMoreInfo);
  }
  void _scrollToBottom() { if (!_scrollController.hasClients) return; WidgetsBinding.instance.addPostFrameCallback((_) { if (_scrollController.hasClients) { final maxScroll = _scrollController.position.maxScrollExtent; final currentScroll = _scrollController.position.pixels; if (maxScroll - currentScroll < 50.0) { _scrollController.animateTo(maxScroll, duration: const Duration(milliseconds: 300), curve: Curves.easeOut); } } }); }
